...
pub struct Session {
    ...
    last_wa_verified: Option<Instant>,  // Set when WebAuthn ceremony completes
}
...
impl Session {
    ...
    /// Record that a WebAuthn ceremony was successfully completed
    pub fn set_wa_verified(&mut self) {
        self.last_wa_verified = Some(Instant::now());
    }

    /// Check if WebAuthn was verified within the given number of seconds.
    /// Consumes the verification on success (single-use). An attacker
    /// cannot piggyback on a legitimate user's verification window.
    pub fn consume_wa_verification(&mut self, secs: u64) -> bool {
        match self.last_wa_verified.take() {
            Some(t) if t.elapsed() < Duration::from_secs(secs) => true,
            _ => false,
        }
    }
