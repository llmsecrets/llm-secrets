# Bash completion for scrt
# Install: source this file or copy to /etc/bash_completion.d/

_scrt_completions() {
    local cur prev commands
    COMPREPLY=()
    cur="${COMP_WORDS[COMP_CWORD]}"
    prev="${COMP_WORDS[COMP_CWORD-1]}"

    # Main commands
    commands="unlock status list run add view extend logout lock clear check-hello check migrate backup-key help"

    case "$prev" in
        scrt)
            COMPREPLY=($(compgen -W "$commands" -- "$cur"))
            return 0
            ;;
        unlock|extend)
            # TTL in seconds - suggest common values
            COMPREPLY=($(compgen -W "1800 3600 7200 14400 28800" -- "$cur"))
            return 0
            ;;
        run)
            # Complete file names for commands
            COMPREPLY=($(compgen -c -- "$cur"))
            return 0
            ;;
        add)
            # No completion for KEY=value pairs
            return 0
            ;;
        migrate)
            # No completion for master key
            return 0
            ;;
        *)
            # If we're in a run command, complete file names
            for ((i=1; i < COMP_CWORD; i++)); do
                if [[ "${COMP_WORDS[i]}" == "run" ]]; then
                    COMPREPLY=($(compgen -c -- "$cur"))
                    return 0
                fi
            done
            ;;
    esac

    return 0
}

complete -F _scrt_completions scrt
